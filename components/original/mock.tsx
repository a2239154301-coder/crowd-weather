// @ts-nocheck
/* ============================================================
   馬場氏の原案モック（受領時のまま・無改変で保存）
   比較用の「Before」。ここは直さない。
   - askOrca() が now.shadeRate / plan.peakH という存在しない項目を参照している
     （undefined はJSON化で消えるため、日陰率とピーク時刻がAIに渡らない）
   - フッターは「都知事杯オープンデータ・ハッカソン 2026」表記のまま
   いずれも当時の状態を保つために意図的に残している。
   ============================================================ */
"use client";
...
export default function CrowdWeather() {
  ...
  const askOrca = async () => {
    ...
    try {
      ...
      setAiAdvice("AI提案の取得に失敗しました。VercelのORCAROUTER_API_KEY設定を確認してください。");
    } finally {
      setAiLoading(false);
    }
  };
