...
export default function ArrivalList({ arrivals, hour }: { arrivals: ZoneRisk[]; hour: number }) {
  ...
  return (
    <div style={{ background: T.surface, border: `1px solid ${T.line}`, borderRadius: 12, padding: "13px 16px" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap", marginBottom: 4 }}>
        <span style={{ fontSize: 13, letterSpacing: 1, color: T.textFaint }}>
          危険の到来順（{hour}:00 以降）
        </span>
        <SourceTag kind="calc" tone={name === "night" ? "ink" : "day"} />
      </div>
      {arrivals.map((r) => {
