...
**🔗 https://crowd-weather-six.vercel.app/**

**合言葉: `crowdweather2026`**

初回アクセス時は `/gate` へ自動で移動するので、上記の合言葉を入力してください
（以後2週間はCookieで保持されます）。

ゲートを設けているのは、`/api/*` を無認証で公開するとOrcaRouterのクレジットを
第三者に消費されるためです。**デモ期間の保護であって本番の認証ではありません**
（`middleware.ts`）。
